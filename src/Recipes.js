...
const Recipes = ({ recipes, users }) => {
  return (
    <div className="listAll">
      ...
      <ul>
        {
          recipes.map(recipe => <li key={recipe.id} className={"lists"}>{recipe.imageURL}<br />Dish: <Link to={`/recipes/${recipe.id}`} className={"underline"}>{recipe.name}</Link><br />Region: {recipe.cusine}<br />Directions: {recipe.directions}<br />Health Score: {recipe.healthscore}<br />Ingredients: {recipe.ingredients}<br />Created By: {users.map(user => user.id === recipe.userId ? <Link key={user.id} to={`/users/${user.id}`} className={"underline"}>{user.username}</Link> : '')}</li>)
        }
      </ul>
    </div>
  )
